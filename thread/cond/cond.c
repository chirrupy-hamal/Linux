...
void *eat_noodle(void *arg)
{
  ...
  while(1)
  {
    ...
    if(0 == _have_noodle)
    {
      ...
      pthread_cond_wait(&cond, &mutex);//集合了3个操作，其中解锁和休眠是原子操作
    }
...
//一个厨师和一个顾客是不会发生问题的，但是多个厨师和多个顾客就会发生
//做面/吃面逻辑错误。
void *cook_noodle(void *arg)
{
  ...
  while(1)
  {
    ...
    _have_noodle = 1;
    pthread_cond_signal(&cond);//唤醒吃面的人
...
int main()
{
  ...
  //假设顾客1拿到时间片，随后加锁，其他顾客不能加锁，所以其他顾客陷入互斥锁加锁的等待，顾客1吃完之后，唤醒厨师、解锁
  //解锁之后，问题来了，厨师1、顾客2、3、4都能抢，这时候假设顾客2抢到锁，接下来顾客2开始吃面，此时吃面逻辑就错误了。
  //但是程序是不会卡死的。只是发生吃/做面逻辑的错误。
